...
class PSO_full():
    ...
    def __init__(self):
        ...
        self.random_integer_current_best = 1 #c1
        self.random_integer_best_ever = 1 #c2
    ...
    def particle_swarm_optimiser(self):
        ...
        for x in range (0, pso.num_freqs):
          ...
          for y in range(0, self.num_epochs):
            ...
            pso.filename = os.getcwd()+f'/PSO/{self.dataset}/{str((pso.f+(x*pso.increment))/1e9)}_data.csv'
            print(pso.filename)
            for z in range(0, self.num_generations):
              pso.calculate_fitness(freq)
